...
### Repair Pass 191: Vault Velocity Setup Warning

Status: completed in local source.

- Added a Vault presentation-model warning that maps invalid mortgage Velocity projections into coach-tone setup copy.
- Missing LOC terms now produce a top-level Vault warning before the mortgage wizard: users see that known LOC or HELOC limit, APR, fees, and draw rules are required before trusting the mortgage velocity path.
- The warning also covers no-capacity, over-limit, cash-flow, payment-below-interest, and projection-horizon failure reasons without creating payoff claims.
...
### Repair Pass 193: Portfolio Strategy Badge Fit

Status: completed in local source.

- Changed the Portfolio payoff strategy cards so the strategy label and Velocity status badge sit in a compact wrapping header row.
- Moved strategy description copy onto its own readable line so "Velocity Mode" and review/modeling badges no longer compete with explanatory text in narrow panels.
...
### Repair Pass 195: Simulator LOC Balance Control

Status: completed in local source.

- Added a visible LOC Balance editor to the Simulator Line of Credit card, between Limit and APR.
- Wired the control to `store.updateLOC({ balance })`, matching the same LOC balance already used by Simulator projections, LOC interest math, and warning states.
- Added regression coverage proving the Simulator exposes a screen-reader label for LOC balance and keeps the editable control aligned with the projection/warning input.
...
### Repair Pass 196: Mobile Shell Token Controls

Status: completed in local source.

- Replaced the rendered mobile bottom-navigation glyph surface with deterministic route tokens (`DB`, `SIM`, `CK`, `LRN`, `PF`, `WT`, `SET`, `AI`) while preserving accessible route names.
- Replaced the rendered financial domain selector and domain dropdown glyph surfaces with deterministic domain/subcategory tokens derived from stable route/domain labels.
...
### Repair Pass 200: Expo Money Loop Pressure Parity

Status: completed in local source.

- Added bounded `pressurePercent` values to each shared mobile Money Loop step so the Expo shell can visualize loop pressure from the financial engine instead of component-local math.
- Added a native `MobileMoneyLoopPressureStrip` beneath the mobile payoff orbit with five stable progressbar segments for Income, LOC, Expenses, Cash Flow, and Principal.
- Kept the dashboard to the required four vitals; the pressure strip is supporting visual context, not a new KPI.
...
### Repair Pass 202: Portfolio Percent-Minimum Recalculation

Status: completed in local source.

- Fixed Portfolio percent-based minimum payments so they recalculate from the current simulated debt balance instead of the original starting balance.
- Reworked monthly surplus calculation to use current active minimum payments, which naturally releases cash flow as balances decline or debts are paid off.
- Kept fixed-payment behavior unchanged while routing split allocations, Velocity targeting, Money Loop target payments, and other-active-payment calculations through the current-balance minimum helper.
- Added regression coverage proving a percent minimum drops from `$100` to `$90` after the simulated balance drops from `$1,000` to `$900`, while the monthly cash-flow pool remains fully assigned.
- Verification passed with `npm run test` in `apps/web`.

### Repair Pass 203: Multi-Debt Non-Finite Boundary Guard

Status: completed in local source.

- Hardened the web multi-debt simulation boundary so non-finite income, expense, debt balance, APR, payment, term, LOC, and chunk inputs are sanitized before baseline comparisons or monthly payoff math run.
- Reused the shared local debt sanitizer in warning generation and multi-debt simulation so warnings and payoff results agree on the same safe values.
- Kept corrupted multi-debt payoff claims blocked instead of allowing `NaN` or `Infinity` to leak into totals, debt results, warnings, LOC interest, or Money Loop ledgers.
- Added regression coverage proving a directly corrupted `simulateMultiDebt` result serializes without `NaN` or `Infinity` and returns an invalid payoff state.
- Verification passed with `npm run test` in `apps/web`.

### Repair Pass 204: Live Store Non-Finite Setter Guard

Status: completed in local source.

- Hardened Portfolio live setters so direct UI/action updates for income, expenses, extra payment, chunk amount, LOC fields, and split ratio reuse the same finite non-negative guards used during import and hydration.
- Hardened the main financial store setters and partial updates so dashboard/cockpit debt, LOC, mortgage, income, expense, age, and chunk fields cannot persist `NaN` or `Infinity` into state before payoff helpers read them.
- Kept invalid non-finite updates on existing values and continued clamping negative money fields to zero, preserving coachable review states without leaking impossible display values.
- Added regression coverage for live Portfolio and Financial store action paths, proving corrupted setter inputs stay finite before recompute and payoff helper reads.
- Verification passed with `npm run test` in `apps/web`.

### Repair Pass 205: Vault Strategy Label Non-Finite Guard

Status: completed in local source.

- Moved Vault mortgage strategy month and interest labels onto tested model helpers instead of page-only interpolation.
- Hardened Vault strategy savings, time-delta, month, and interest formatting so a corrupted strategy object cannot render `NaN`, `Infinity`, or a malformed payoff claim just because `isPayoffPossible` is truthy.
- Preserved invalid-path language: non-finite projected months show `Review inputs`, and non-finite projected savings or interest show `Not projected`.
- Added regression coverage for corrupted "possible" Vault strategy projections, including non-finite savings, time saved, months, and total interest values.
- Verification passed with `npm run test` in `apps/web`.

### Repair Pass 206: Simulator And Vault Percent Label Guard

Status: completed in local source.

- Added tested percent-label helpers for Simulator and Vault so textual percentage claims use the same non-finite boundary discipline as visual bar widths.
- Routed Simulator payment split and equity labels through the safe formatter instead of direct `.toFixed()` calls.
- Routed Vault payment split, equity, mortgage-history year, and investment-rate labels through safe percent/year helpers.
- Kept non-finite display states coachable with `Review inputs` instead of leaking `NaN%`, `Infinity%`, or `NaN years`.
- Added regression coverage proving Simulator and Vault page sources use the safe helpers for these labels and do not directly format the guarded percentage fields.
- Verification passed with `npm run test` in `apps/web`.

### Repair Pass 207: Portfolio Live Debt Sanitization Guard

Status: completed in local source.

- Hardened Portfolio live `addDebt` and `updateDebt` actions so debt balance, APR, minimum rules, term, promo fields, category, kind, and payment source are sanitized before route rendering or recompute.
- Preserved strict import behavior while adding fallback-preserving sanitization for live edits, so a bad UI/action payload cannot leave `NaN`, `Infinity`, invalid categories, or invalid payment-source values in store state.
- Kept percent minimum and promo labels finite before Portfolio page `Math.round(...)` display paths read the debt.
- Expanded live Portfolio regression coverage to corrupt both an existing debt update and a new debt add, then verify exported state contains no `NaN` or `Infinity`.
- Verification passed with `npm run test` in `apps/web`.

### Repair Pass 208: Portfolio Percent Label Guard

Status: completed in local source.

- Added a Portfolio percent-label helper so minimum-payment percent, promo APR, and split-allocation labels reject non-finite values before display.
- Replaced direct `Math.round(... * 100)%` labels in Portfolio route rendering with the safe helper.
- Kept corrupted percentage labels coachable with `Review inputs` instead of leaking `NaN%` or `Infinity%`.
- Added regression coverage proving Portfolio route source uses the helper for percent minimum, promo, and split-ratio labels and no longer formats those labels directly.
- Verification passed with `npm run test` in `apps/web`.
...
- Add math tests. Status: broad route and engine regression coverage exists; Repair Pass 201 added a direct Portfolio simulation corruption fixture, Repair Pass 202 added a current-balance percent-minimum fixture, Repair Pass 203 added a direct multi-debt corruption fixture, Repair Pass 204 adds live store setter corruption fixtures, Repair Pass 205 adds Vault strategy label corruption fixtures, Repair Pass 206 adds Simulator/Vault percent-label corruption fixtures, Repair Pass 207 expands live Portfolio debt corruption fixtures, and Repair Pass 208 adds Portfolio percent-label display coverage.
- Fix invalid/negative cash-flow handling. Status: expanded across Dashboard, Simulator, Vault, mobile snapshots, Portfolio, and multi-debt simulation; Repair Pass 201 hardens Portfolio against non-finite/corrupt inputs before cash-flow and payoff math, Repair Pass 203 does the same for web multi-debt payoff math, Repair Pass 204 closes live store setter paths before bad values reach payoff helpers, Repair Pass 205 keeps Vault labels from trusting corrupt payoff flags, Repair Pass 206 keeps Simulator/Vault percentage labels from leaking non-finite claims, Repair Pass 207 keeps live Portfolio debt edits finite before display or recompute, and Repair Pass 208 keeps Portfolio percentage labels finite at the route boundary.
...
- Add algorithmic payoff visuals. Status: started with the dashboard Money Loop artifact rail in Repair Pass 22, expanded with the Portfolio payoff path SVG in Repair Pass 153, and carried into the Expo native Portfolio path in Repair Pass 154. Repair Pass 197 adds selected-artifact depth layers and rendered desktop/mobile smoke for the dashboard Money Loop carousel, Repair Pass 199 adds a model-bound SVG pressure path to the dashboard Money Loop orbit, and Repair Pass 200 carries bounded loop-pressure context into the Expo dashboard without adding another vital. Deeper full-scene 3D and Remotion/HyperFrames explainers remain open.
