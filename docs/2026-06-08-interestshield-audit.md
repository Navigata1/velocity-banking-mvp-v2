...
- Updated the manual `Mobile iOS smoke` workflow so it still attempts the real Expo Go Simulator smoke twice, but defaults to validating the iOS bundle export when GitHub-hosted Simulator launch fails.
- Added a `require_simulator` workflow input for strict simulator-only runs on a healthy macOS/Xcode host.
- Added iOS bundle fallback artifact upload so hosted runs can keep the generated native export evidence.
- Added mobile contract coverage for the new package script, bundle smoke validator, strict workflow input, fallback copy, bundle output directory, timeout, and artifact upload.
