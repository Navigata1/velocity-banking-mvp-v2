...
export function DepositReservoirMesh({ artifact, isActive, position, onSelect }: ArtifactMeshProps) {
  return (
    <mesh position={position} scale={isActive ? 1.1 : 0.82} onPointerDown={createArtifactSelectionHandler(artifact.id, onSelect)}>
...
export function CreditApertureMesh({ artifact, isActive, position, onSelect }: ArtifactMeshProps) {
  return (
    <mesh position={position} rotation={[Math.PI / 2, 0, 0]} scale={isActive ? 1.1 : 0.82} onPointerDown={createArtifactSelectionHandler(artifact.id, onSelect)}>
...
export function FlowCoreMesh({ artifact, isActive, position, onSelect }: ArtifactMeshProps) {
  return (
    <mesh position={position} scale={(isActive ? 1.1 : 0.82) * (0.8 + artifact.channels.energy * 0.28)} onPointerDown={createArtifactSelectionHandler(artifact.id, onSelect)}>
...
export function PrincipalShieldMesh({ artifact, isActive, position, onSelect }: ArtifactMeshProps) {
  return (
    <mesh position={position} rotation={[0, 0, Math.PI / 4]} scale={isActive ? 1.1 : 0.82} onPointerDown={createArtifactSelectionHandler(artifact.id, onSelect)}>
